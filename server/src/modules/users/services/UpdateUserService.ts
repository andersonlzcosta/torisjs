...
class UpdateProfileService {
    public async execute({ userId, nome, credencial, email, emailAlternativo, nascimento, cargo, telefone1, telefone2, profissao, old_password, password, abrigoId, recoveryCode }: Request): Promise<User | undefined> {
        ...
        if (!recoveryCode) recoveryCode = user.recoveryCode;
            
        return usersRepository.update(userId, { nome, credencial, email, emailAlternativo, nascimento, cargo, telefone1, telefone2, profissao, password, abrigoId, recoveryCode });
    }
}
